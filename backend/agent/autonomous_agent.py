...
5. **截图类任务（截屏、截图桌面等）：一旦有一次成功的截图结果，立即输出 finish，不要重复截图**
6. **优先使用批量命令（如 mv *.txt dest/）而不是逐个操作**
7. **保持简洁高效，避免不必要的步骤**
8. **发送邮件：必须使用 call_tool(tool_name=mail)。禁止使用 open_app 打开 Mail 应用来发邮件（Mail 应用限制多、无法可靠自动化）。**
