...
11. **call_tool** - 调用已注册的内置工具（推荐用于截图、联网搜索、技能库、邮件等）
   - **联网搜索**（查财报、新闻、实时数据）：{"action_type": "call_tool", "params": {"tool_name": "web_search", "args": {"action": "search|news|get_stock", "query": "搜索词", "language": "zh-CN"}}, "reasoning": "..."}。你有 web_search 工具可获取实时信息，研究、调研、查最新数据时请先调用它，不要以「无法获取实时数据」为由拒绝。
