...
11. **call_tool** - Invoke a registered built-in tool (recommended for screenshots, web search, capsules, email, etc.)
   - **Web search** (financials, news, real-time data, weather): {"action_type": "call_tool", "params": {"tool_name": "web_search", "args": {"action": "search|research|news|get_stock|get_weather", "query": "search terms", "language": "zh-CN"}}, "reasoning": "..."}. Use `action="research"` for multi-source research because it returns compact findings plus extracted page evidence. Never refuse by saying "I cannot get real-time data". For weather, use action="get_weather" with city name as query.
