...
- request_tool_upgrade: {"tool": "request_tool_upgrade", "args": {"reason": "需要升级的原因"}} — 用户要创建新工具/监控脚本/Agent能力时调用，不要用 file_operations 直接写
...
2. 简单问答直接回复，不需要 JSON
3. 每次只调用一个工具
4. 邮件未配置账户时：可询问用户提供邮箱密码，用 input_control 自动输入完成配置
5. 用户要创建新工具/监控/Agent 可调用能力时，调用 request_tool_upgrade，不要用 file_operations 写脚本
