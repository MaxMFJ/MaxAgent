...
|------|------|----------------|----------------------|
| **状态** | v3 + v3.1（memory/规划/escalation/反思/安全/trace/首步加固/MACAGENT.md）+ **v3.2（Trace完善/Traces API/health deep/Phase C 全部落地）** | 可观测（trace+执行轨迹）✅ + 可评估（benchmark 自动化）✅ + `/health/deep` ✅；沙箱、审计、HITL、回滚留 v3.3 | 主循环三阶段（含显式 Verify）待做；Extended Thinking ✅；会话 resume/fork 留 v3.3；Subagent Flag 已预留 |
| **里程碑** | Phase A（文档+稳定性）✅ Phase B（体验对齐）✅ Phase C（研究增强）✅ | v3.2 全部完成 ✅ | v3.3 规划中（HITL/沙箱/幂等/Subagent）|
...
| [v3.2_PLAN.md](v3.2_PLAN.md) | **v3.2 功能清单：Trace完善、Traces API、/health/deep、Phase C、FeatureFlag 说明** |
