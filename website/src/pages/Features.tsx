...
import { MessageSquare, Brain, Mic, BarChart3, Settings, Cloud, Zap, Shield } from 'lucide-react'
...
const features = [
  {
    icon: MessageSquare,
    title: '流式对话 (ReAct)',
    desc: '基于 ReAct 推理链的流式对话：Agent 按需调用工具（文件、终端、应用、截图、邮件等），支持断线重连与输出恢复。三阶段 Prompt（Gather → Act → Verify）确保执行质量。',
  },
  {
    icon: Brain,
    title: '自主任务 (Autonomous)',
    desc: '长 horizon 多步执行：可选反思（失败分类 + 专用 prompt）、模型选择（本地/云端按任务复杂度）、自适应停止（任务完成/无进展/循环检测）。支持 Plan-and-Execute 模式。',
  },
  {
    icon: Mic,
    title: '语音与 TTS',
  ...
  },
  {
    icon: BarChart3,
    title: '监控仪表板',
    desc: '执行时间线、系统状态、历史分析、实时日志流；Token/RPM/TPM 统计、模型分布；v3.2 Trace 级 span 统计与执行轨迹持久化。',
  },
  {
    icon: Settings,
    title: '权限管理',
      ...
      <div className="mx-auto max-w-4xl">
        ...
        >
          ...
          <p className="text-lg text-[var(--text-muted)]">
            Chow Duck 提供完整的 Agent 对话、执行、监控与扩展能力。支持 Mac 客户端与 Web 端多端同步，Mac/iOS 按 session 共享会话。
          </p>
        </motion.div>
